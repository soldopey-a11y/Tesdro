...
import { API, fmt } from '../lib/ansdropUtils';
...
export default function Admin() {
  ...
  async function verifyAndLoad(candidate) {
    ...
    setLoading(true);
    try {
      const r = await axios.post(`${API}/admin/ping`, {}, { headers: adminHeaders(candidate) });
      if (r.status !== 200) {
        setAuthed(false);
  ...
  async function loadWinners(key = pass) {
    setLoading(true);
    try {
      const r = await axios.post(`${API}/admin/winners`, {}, { headers: adminHeaders(key) });
  ...
  async function loadStatus() {
    try {
      ...
      setSystemStatus(r.data?.systemStatus || 'stopped');
    } catch (e) {}
  }
  ...
  async function startSystem() {
    ...
    try {
      await axios.post(`${API}/admin/start`, {}, { headers: adminHeaders(pass) });
      await loadStatus();
  ...
  async function confirmReset() {
    ...
    try {
      await axios.post(`${API}/admin/reset`, {}, { headers: adminHeaders(pass) });
      await loadWinners();
      await loadStatus();
  ...
  async function markPaid(winnerId) {
    ...
    try {
      await axios.post(
        `${API}/admin/mark-paid`,
        { winnerId, txHash: (txDrafts[winnerId] || '').trim() },
  ...
  async function unmarkPaid(winnerId) {
    if (!window.confirm('Unmark this winner as paid?')) return;
    setBusyId(winnerId);
    try {
      await axios.post(`${API}/admin/unmark-paid`, { winnerId }, { headers: adminHeaders(pass) });
      await loadWinners();
    } finally {
      setBusyId(null);
    }
        ...
        >
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] uppercase tracking-[0.28em] text-white/50">internal only</div>
            </div>
          </div>
          <label className="mb-1 block text-[10px] font-semibold uppercase tracking-widest text-white/50">Password</label>
        ...
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] uppercase tracking-[0.28em] text-white/40">Winner payouts</div>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        ...
        <div className="mb-6 rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.04] to-transparent p-5 backdrop-blur-md">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {startLoading ? 'Starting…' : running ? 'System Running' : 'Start System'}
              </button>
              <button
          ...
          <div className="mt-3 text-[11px] text-white/50">
            ...
            <span className="font-semibold text-rose-300/80">Reset</span> wipes every winner record and stops the system — the public page will show “NOT STARTED YET”.
          </div>
        </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          {winners?.map((w) => (
            <div key={w.id} className={`rounded-2xl border p-4 transition ${w.paid ? 'border-emerald-400/25 bg-emerald-500/[0.03]' : 'border-white/10 bg-white/[0.03]'}`}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 flex items-center gap-2">
                    <div className="break-all font-mono text-sm font-semibold text-white sm:text-base">{w.address}</div>
                    <button onClick={() => copyText(w.address, 'addr-' + w.id)} title="Copy address" className="shrink-0 rounded p-1 text-white/50 hover:bg-white/5 hover:text-white">
                      {copied === 'addr-' + w.id ? <Check className="h-3.5 w-3.5 text-emerald-300" /> : <Copy className="h-3.5 w-3.5" />}
                    </button>
                  </div>
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
                    <span className="rounded-full border border-amber-400/40 bg-amber-500/10 px-2 py-0.5 font-mono font-bold text-amber-200">{w.crashPoint.toFixed(2)}x</span>
                    <span className="rounded-full border border-emerald-400/40 bg-emerald-500/10 px-2 py-0.5 font-mono font-bold text-emerald-200">Send {fmt(w.tokensWon)} $ANSEM</span>
                    <button onClick={() => copyText(String(w.tokensWon), 'amt-' + w.id)} title="Copy amount" className="rounded p-1 text-white/50 hover:bg-white/5 hover:text-white">
                      {copied === 'amt-' + w.id ? <Check className="h-3.5 w-3.5 text-emerald-300" /> : <Copy className="h-3.5 w-3.5" />}
                    </button>
                  </div>
                </div>
                  ...
                  {w.paid ? (
                    <div className="flex flex-col items-stretch gap-2 sm:items-end">
                      <div className="inline-flex items-center gap-1.5 self-start rounded-full border border-emerald-400/40 bg-emerald-500/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-emerald-300 sm:self-end">
                        ...
                        {w.paidAt && <span className="ml-1 text-emerald-300/60">{new Date(w.paidAt).toLocaleTimeString()}</span>}
                      </div>
                      {w.txHash && (
                        <a href={`https://solscan.io/tx/${w.txHash}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 self-start text-xs text-cyan-300 hover:underline sm:self-end">
                          <ExternalLink className="h-3 w-3" />
                          {w.txHash.slice(0, 10)}…{w.txHash.slice(-8)}
                        </a>
                      )}
                      <button onClick={() => unmarkPaid(w.id)} disabled={busyId === w.id} className="self-start text-[11px] text-rose-400 hover:underline sm:self-end">Unmark</button>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-2">
                      <input
                        ...
                        value={txDrafts[w.id] || ''}
                        onChange={(e) => setTxDrafts((d) => ({ ...d, [w.id]: e.target.value }))}
                      ...
                      />
                      <button onClick={() => markPaid(w.id)} disabled={busyId === w.id} className="flex items-center justify-center gap-1.5 rounded-lg bg-gradient-to-r from-emerald-500 to-cyan-500 px-3 py-2 text-xs font-bold text-white shadow-[0_0_20px_rgba(16,185,129,0.35)] hover:opacity-90 disabled:opacity-50">
        ...
        <div className="mt-8 rounded-2xl border border-white/10 bg-black/40 p-4 text-xs text-white/50">
          <div className="mb-1 font-semibold uppercase tracking-widest text-white/70">How to mark paid</div>
          <ol className="ml-4 list-decimal space-y-1">
            ...
            <li>Paste it into the input on the winner's row and click <span className="font-semibold text-emerald-300">Mark as Paid</span>.</li>
            <li>The <span className="font-semibold text-emerald-300">Paid</span> badge will instantly appear on the public site.</li>
          </ol>
        </div>
      </main>
